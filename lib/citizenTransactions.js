...
class citizenTransactions extends Contract {
    ...
    // citizenExists returns true when citizen with given NIC exists in world state.
    ...
    // get all results of citizen history as JSON Array
		...
		while (!res.done) {
			if (res.value && res.value.value.toString()) {
				...
				if (isHistory && isHistory === true) {
					...
					jsonRes.Timestamp = new Date(res.value.timestamp).toISOString();
					try {
  ...
  async makeRequest(ctx, NIC, variables, requestedby, reqid) {
    const genid = reqid;
  ...
  async viewPendingRequests(ctx, NIC) {
    ...
    return jsonresult.filter((obj) => obj.status === "pending");
  }
  ...
  async viewCitizenInfo(ctx, requestid) {
    ...
    if (!requestJSON || requestJSON.length === 0) {
      ...
      if (requestJSON.status === "accepted") {
        const citizenJSON = await ctx.stub.getState(NIC); // get the citizen from chaincode state
        if (!citizenJSON || citizenJSON.length === 0) {
          throw new Error(`The citizen ${NIC} does not exist`);
        }
		//get citizen custom info
		const customJSON = citizenJSON.map(obj => {
			const newObj = {};
			for (const variable of requestid.variables) {
